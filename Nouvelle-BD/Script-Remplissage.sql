...
FROM fievetl.COURS_ENSEIGNANTS_DATA
WHERE IDENSEIGNANTRESPONSABLE NOT IN (SELECT DISTINCT IdUtilisateur FROM fievetl.Utilisateurs)
...
FROM FIEVETL.COURS_ENSEIGNANTS_DATA
WHERE IDINTERVENANT NOT IN (SELECT DISTINCT IdUtilisateur FROM fievetl.Utilisateurs)
