...
SELECT DISTINCT te3.idCours, te3.IDENSEIGNANTRESPONSABLE, te3.idSemestre, te3.idModule
FROM (SELECT idCours,
...
SELECT DISTINCT IDEVALUATION, COEFFICIENT, IDUTILISATEUR
FROM fievetl.NOTES_DATA
WHERE IDEVALUATION IS NOT NULL AND IDEVALUATION IN
    (SELECT DISTINCT IDEVALUATION FROM FIEVETL.NOTES_DATA MINUS SELECT DISTINCT IDEVALUATION FROM FIEVETL.EVALUATIONS);
commit;
...
--etre_absent--
INSERT INTO FIEVETL.EtreAbsent (IdEtudiant, Dates, IdCours, Matin, Justifiee, MotifAbsence, EstAbsent)
