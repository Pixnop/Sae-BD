...
WHERE abs.DATES BETWEEN '01/09/2019' AND '01/01/2020' AND VueNbrAbsJust.DATES BETWEEN '01/09/2019' AND '31/12/2019' AND c.IDSEMESTRE = 1
...
-- Toutes les infos des étudiants de l'année 2020

CREATE OR REPLACE VIEW FIEVETL.VueInfosEtudiant2020 AS
SELECT e.IDETUDIANT, e.CIVILITÉ, e.NOMETUDIANT, e.PRENOMETUDIANT, g.NOMGROUPE, COUNT(abs.idabsence) AS NombreAbsences, VueNbrAbsJust, vueMoyenneEtudiant2020S1, a.APPELATIONBAC, a.NOMSPECIALITE
FROM FIEVETL.ETUDIANTS e
         JOIN FIEVETL.ADMISSIONS a ON e.IDADMISSION = a.IDADMISSION
         JOIN FIEVETL.GROUPES g ON e.IDGROUPE = g.IDGROUPE
         JOIN FIEVETL.EtreAbsent abs ON e.IDETUDIANT = abs.IDETUDIANT
         ...
         JOIN FIEVETL.ETUDIANTCOURS ec ON e.IDETUDIANT = ec.IDETUDIANT
         JOIN FIEVETL.COURS c ON ec.IDCOURS = c.IDCOURS
WHERE abs.DATES BETWEEN '01/09/2020' AND '01/01/2021' AND VueNbrAbsJust.DATES BETWEEN '01/09/2019' AND '01/01/2021' AND c.IDSEMESTRE = 1
